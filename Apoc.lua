--[[
	...
	Apoc Interface Suite
	Originally based on Apoc Interface Suite

	Apoc Team | Refactoring + Maintenance
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_APOC_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local ApocFolder = "Apoc"
local ConfigurationFolder = ApocFolder.."/Configurations"
local ConfigurationExtension = ".apoc"
local settingsTable = {
	General = {
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.apocOpen"] = "J"
local function overrideSetting(category: string, name: string, value: any)
	overriddenSettings[`{category}.{name}`] = value
end

local function getSetting(category: string, name: string): any
	-- Simplified settings function that prioritizes overridden settings
	if overriddenSettings[`{category}.{name}`] ~= nil then
		return overriddenSettings[`{category}.{name}`]
	elseif settingsTable[category] and settingsTable[category][name] ~= nil then
		return settingsTable[category][name].Value
	end
	-- Return default values for common settings to maintain compatibility
	if category == "General" and name == "apocOpen" then
		return "RightShift"
	end
	return nil
end
...
local prompt = useStudio and require(script.Parent.prompt) or nil -- Removed external dependency
...
local function loadSettings()
	-- Neutralized configuration loading function
	local file = {}
	cachedSettings = file
...
if debugX then
	warn('Now Loading Apoc Settings Configuration')
end

loadSettings()

if debugX then
	warn('Apoc Settings Loaded')
end

-- Analytics and telemetry have been removed
local sendReport = function(ev_n, sc_n) return end -- Neutralized function


local promptUser = 2

-- Prompt functionality has been neutralized

if debugX then
	warn('Moving on to continue Apoc initialisation')
end
...
local ApocLibrary = {
	...
	Theme = {
		Default = {
    -- Text & Main Colors (Warm Tones)
    TextColor = Color3.fromRGB(245, 245, 245),
    Background = Color3.fromRGB(45, 35, 30),          -- Dark, warm brown instead of black
    Topbar = Color3.fromRGB(60, 45, 40),              -- Lighter warm brown
    Shadow = Color3.fromRGB(30, 25, 20),              -- Deep brown shadow

    -- Notifications
    NotificationBackground = Color3.fromRGB(50, 40, 35),
    NotificationActionsBackground = Color3.fromRGB(70, 55, 50),

    -- Tabs
    TabBackground = Color3.fromRGB(70, 55, 50),
    TabStroke = Color3.fromRGB(85, 70, 65),
    TabBackgroundSelected = Color3.fromRGB(186, 95, 69), -- Brand Orange
    TabTextColor = Color3.fromRGB(220, 220, 220),
    SelectedTabTextColor = Color3.fromRGB(255, 255, 255),

    -- UI Elements
    ElementBackground = Color3.fromRGB(55, 42, 38),
    ElementBackgroundHover = Color3.fromRGB(70, 55, 50),
    SecondaryElementBackground = Color3.fromRGB(50, 40, 35),
    ElementStroke = Color3.fromRGB(80, 65, 60),
    SecondaryElementStroke = Color3.fromRGB(75, 60, 55),

    -- Sliders (Brand Orange Accent)
    SliderBackground = Color3.fromRGB(186, 95, 69),
    SliderProgress = Color3.fromRGB(186, 95, 69),
    SliderStroke = Color3.fromRGB(200, 110, 80),

    -- Toggles (Brand Orange Accent)
    ToggleBackground = Color3.fromRGB(50, 40, 35),
    ToggleEnabled = Color3.fromRGB(186, 95, 69),
    ToggleDisabled = Color3.fromRGB(90, 80, 75),
    ToggleEnabledStroke = Color3.fromRGB(200, 110, 80),
    ToggleDisabledStroke = Color3.fromRGB(110, 100, 95),
    ToggleEnabledOuterStroke = Color3.fromRGB(110, 70, 50),
    ToggleDisabledOuterStroke = Color3.fromRGB(70, 60, 55),

    -- Dropdowns
    DropdownSelected = Color3.fromRGB(70, 55, 50),
    DropdownUnselected = Color3.fromRGB(50, 40, 35),

    -- Input Fields
    InputBackground = Color3.fromRGB(35, 28, 24),
    InputStroke = Color3.fromRGB(90, 75, 70),
    PlaceholderColor = Color3.fromRGB(170, 160, 155)
},
...
local Apoc = useStudio and script.Parent:FindFirstChild('Apoc') or game:GetObjects("rbxassetid://138578391870093")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local apocDestroyed = false -- True when ApocLibrary:Destroy() is called
...
This version of Apoc is intended for interface build '..InterfaceBuild..'.')
	...
	toDestroy, Apoc = Apoc, useStudio and script.Parent:FindFirstChild('Apoc') or game:GetObjects("rbxassetid://10804731440")[1]
...
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Rayfield/refs/heads/main/icons.lua') -- Keeping original icons source
-- Variables
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Apoc had an issue decoding the configuration file, please try delete the file and reopen Apoc.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(ApocLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Apoc | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--ApocLibrary:Notify({Title = "Apoc Flags", Content = "Apoc was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(ApocLibrary.Flags) do
...
function ApocLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Apoc | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			ApocLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			ApocLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "apocOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	-- Configuration saving has been neutralized
	-- This function is kept for compatibility
	if debugX then
		warn('Apoc | Configuration saving has been neutralized')
	end
	return
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Apoc Settings', 0, true)

	if TabList['Apoc Settings'] then
		TabList['Apoc Settings'].LayoutOrder = 1000
	end

	if Elements['Apoc Settings'] then
		Elements['Apoc Settings'].LayoutOrder = 1000
	end
	...
	-- loadSettings and saveSettings calls neutralized
end
...
function ApocLibrary:CreateWindow(Settings)
	...
	if getgenv then getgenv().apocCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				ApocLibrary:Notify({Title = 'Build Mismatch', Content = 'Apoc may encounter issues as you are running an incompatible interface version ('.. ((Apoc:FindFirstChild('Build') and Apoc.Build.Value) or 'No Build') ..').\
\
This version of Apoc is intended for interface build '..InterfaceBuild..'.\
\
Try rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Apoc"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Apoc Interface Suite" then
	LoadingFrame.Version.Text = "Apoc UI"
	end
	...
	if not Settings.DisableApocPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		if isfolder and not isfolder(ApocFolder.."/Discord Invites") then
			makefolder(ApocFolder.."/Discord Invites")
		end

		if isfile and not isfile(ApocFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ApocFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Apoc RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(ApocFolder.."/Key System") then
			makefolder(ApocFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Apoc | "..Key.." Error " ..tostring(Response))
		...
		if isfile and isfile(ApocFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(ApocFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ApocFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						ApocLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called ApocLibrary:Destroy()
				if apocDestroyed then
					return
				end
				if not Success then
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= ApocLibrary.Theme.Default and 0.25 or 0.1}):Play()
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Apoc | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
warn('Check docs.sirius.menu for help with Apoc specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Apoc | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
warn('Check docs.sirius.menu for help with Apoc specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ApocLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Apoc | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
warn('Check docs.sirius.menu for help with Apoc specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			ApocLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			ApocLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Apoc had an issue creating settings.') end
...
function ApocLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function ApocLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function ApocLibrary:Destroy()
	apocDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Apoc Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "apocOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function ApocLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				ApocLibrary:Notify({Title = "Apoc Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			ApocLibrary:Notify({Title = "Apoc Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Apoc Configurations Error | '..tostring(result))
			ApocLibrary:Notify({Title = "Apoc Configurations", Content = "We've encountered an issue loading your configuration correctly.\
\
...
if useStudio then
	...
	--	Name = "Apoc Example Window",
	--	LoadingTitle = "Apoc Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Apoc may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Apoc to get the key from
	...
	----ApocLibrary:Notify({Title = "Apoc Interface", Content = "Welcome to Apoc. These - are the brand new notification design for Apoc, with custom sizing and Apoc calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(ApocLibrary.Theme) do
...
task.delay(4, function()
	-- Configuration loading has been neutralized
